...
import { createStep, deleteStep, getStepsByWorkspace } from "@/actions/Step";
...
import { capitalizeWords, copyBody } from "@/utils/format";
...
export default function StepManager({ className }: { className?: string }) {
    ...
    const { setEditMode, setStepInEdit, editMode } = useContext(StepContext);
    useEffect((
    ) => {
    ...
    const duplicateStep = async (step: Step) => {
        ...
        const newStep: Omit<Step, "_id"> = {
            workspaceId: step.workspaceId,
            type: step.type,
            apiPath: step.apiPath,
            apiMethod: step.apiMethod,
            body: copyBody(step.body),
            nodeId: step.nodeId,
            order: steps.length
        }
    ...
    return (
        <main className={`h-full bg-white rounded-md p-4 ${className} flex flex-col gap-2 min-w-0 overflow-y-scroll`}>
            <div className="flex items-center justify-between">
                ...
                {!stepAddMode &&
                    <button className={`${isExecuting ? "opacity-50" : ""} justify-center p-1 bg-green-500 text-sm rounded-md hover:bg-white hover:text-green-500 border-2 border-green-500`} type="button"
                    ...
                    <table className="w-full caption-bottom text-sm border-collapse min-w-full">
                        <thead className="[&amp;_tr]:border-b">
                            <tr className="border-b transition-colors hover:bg-muted/50 data-[state=selected]:bg-muted">
                                ...
                                <th className="h-12 px-4 text-left align-middle font-medium text-muted-foreground [&amp;:has([role=checkbox])]:pr-0 text-black border-solid border-b-2 border-gray-500">Edit Step</th>
                                ...
                                <th className="h-12 px-4 text-left align-middle font-medium text-muted-foreground [&amp;:has([role=checkbox])]:pr-0 text-black border-solid border-b-2 border-gray-500">Duplicate Step</th>
                                    ...
                                    <tr className="border-b transition-colors hover:bg-muted/50 data-[state=selected]:bg-muted" key={index}>
                                        ...
                                        <td className="p-4 align-middle [&amp;:has([role=checkbox])]:pr-0 text-black border-solid border-b-2 border-gray-300">{
                                            <button className={`p-1 bg-blue-500 rounded-md text-white text-center w-fit border-2 border-blue-500 hover:bg-white hover:text-blue-500`}
                                                onClick={() => { setEditMode(true); setStepInEdit({ ...step }) }}>
                                                Edit Step
                                            </button>
                                        ...
                                        <td className="p-4 align-middle [&amp;:has([role=checkbox])]:pr-0 text-black border-solid border-b-2 border-gray-300">
                                            <button className={`p-1 bg-yellow-500 rounded-md text-white text-center w-fit border-2 border-yellow-500 hover:bg-white hover:text-yellow-500`}
                                                onClick={async () => await duplicateStep(step)}>
                                                Duplicate
                                            </button>
                                        </td>
